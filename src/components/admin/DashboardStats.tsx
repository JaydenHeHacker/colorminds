...
export default function DashboardStats() {
  const {
    ...
    queryFn: async () => {
      ...
      const {
        count: totalPages
      } = await supabase.from('coloring_pages').select('*', {
        count: 'exact',
        head: true
      });
      ...
      const {
        data: downloads
      } = await supabase.from('coloring_pages').select('download_count');
      const totalDownloads = downloads?.reduce((sum, page) => sum + (page.download_count || 0), 0) || 0;

      // Get series count
      const {
        data: seriesData
      } = await supabase.from('coloring_pages').select('series_id').not('series_id', 'is', null);
      ...
      } = await supabase.from('coloring_pages').select('category_id, categories(name, icon)').not('category_id', 'is', null);
      ...
      const topCategories = Object.entries(categoryStats || {}).sort(([, a]: any, [, b]: any) => b.count - a.count).slice(0, 5).map(([name, data]: any) => ({
        name,
        count: data.count,
        icon: data.icon
      }));
  ...
  return <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {statCards.map(stat => <Card key={stat.title} className="p-6">
      ...
      {stats?.topCategories && stats.topCategories.length > 0 && <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">热门分类 Top 5</h3>
          <div className="space-y-3">
            {stats.topCategories.map((cat, index) => <div key={cat.name} className="flex items-center justify-between">
